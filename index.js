...
bot.on('message', message => {
    ...
    ;(async () => {
        ...
        */
	    ...
	    if (/inv/.test(message.content)) {
			await message.channel.send({
				embed: new Discord.RichEmbed()
		            .setTitle(">> ClickHere To Add" + ' ' + `${bot.user.username}` + " <<")
		            .setURL("https://discordapp.com/oauth2/authorize?client_id=" + `${bot.user.id}` + "&scope=bot&permissions=2080374975")
    		        .setTimestamp()
